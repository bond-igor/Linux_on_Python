...
def deploy_p7zip():
    res = []
    upload_files(data["ip"], data["user"], data["passwd"], f'/home/user/{data["pkgname"]}.deb',
...
@pytest.fixture(autouse=True, scope='module')
def deploy_crc32():
    res = (ssh_checkout(data["ip"], data["user"], data["passwd"], f'echo {data["passwd"]} | sudo apt install {data["pkgname2"]}',
                            "Настраивается пакет"))
    return res
